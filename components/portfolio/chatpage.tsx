...
import { Send, MessageCircle, Loader2, UserCircle, ShieldCheck, Lock, Paperclip, Mic, MicOff, X, Play, Pause, Image as ImageIcon, FileText, Film } from "lucide-react";
            ...
            <div className="ml-2 border-l border-border pl-2">
              {mounted && (
                <button onClick={() => setTheme(resolvedTheme === "dark" ? "light" : "dark")}
                  className="relative p-2 rounded-lg hover:bg-secondary transition-colors text-muted-foreground hover:text-foreground" aria-label="Toggle theme">
...
interface ChatMessage {
  ...
  file_type?: string | null; // "image" | "video" | "audio" | "file"
...
function AudioPlayer({ url }: { url: string }) {
  ...
  return (
    <div className="flex items-center gap-2 min-w-[160px]">
      <button onClick={toggle} className="w-8 h-8 rounded-full bg-primary/20 flex items-center justify-center shrink-0 hover:bg-primary/40 transition-colors">
...
function MessageBubble({ msg, isMe, userName }: { msg: ChatMessage; isMe: boolean; userName: string }) {
  const isAdmin = msg.role === "admin";
  const hasFile = !!msg.file_url;

  return (
    <div className={cn("flex gap-2", isMe ? "flex-row-reverse" : "flex-row")}>
      <div className={cn("w-7 h-7 rounded-full flex items-center justify-center shrink-0 text-xs font-bold mt-1",
        isMe ? "bg-primary/20 text-primary" : "bg-secondary text-foreground")}>
        {isMe ? (userName[0] ?? "U") : isAdmin ? "C" : "AI"}
      </div>
      <div className={cn("flex flex-col max-w-[75%]", isMe ? "items-end" : "items-start")}>
        {isAdmin && <span className="text-[10px] text-muted-foreground mb-0.5 px-1">Clarence</span>}

        {/* File/media content */}
        {hasFile && (
          <div className={cn("mb-1 rounded-2xl overflow-hidden",
            isMe ? "rounded-tr-sm" : "rounded-tl-sm")}>
            {msg.file_type === "image" && (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={msg.file_url!} alt="Image" className="max-w-[220px] max-h-[220px] object-cover rounded-2xl cursor-pointer"
                onClick={() => window.open(msg.file_url!, "_blank")} />
            )}
            {msg.file_type === "video" && (
              <video src={msg.file_url!} controls className="max-w-[220px] rounded-2xl" />
            )}
            {msg.file_type === "audio" && (
            ...
            {msg.file_type === "file" && (
              <a href={msg.file_url!} target="_blank" rel="noopener noreferrer"
                className={cn("flex items-center gap-2 px-3 py-2.5 rounded-2xl text-sm",
                  isMe ? "bg-primary text-white" : "bg-secondary")}>
        ...
        {msg.content && (
          <div className={cn("px-4 py-2.5 rounded-2xl text-sm leading-relaxed",
            isMe ? "bg-primary text-white rounded-tr-sm"
              : isAdmin ? "bg-green-500/10 border border-green-500/30 text-foreground rounded-tl-sm"
...
export function ChatPage() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "chatbot_conversations", filter: `session_id=eq.${sessionId}` },
        (payload) => {
  ...
  const signInWithGoogle = async () => {
    await supabase.auth.signInWithOAuth({
      provider: "google",
      options: { redirectTo: `${window.location.origin}/chat`, queryParams: { prompt: "select_account" } },
    });
  };
  ...
  const sendMessage = async (overrideText?: string, fileBlob?: Blob, blobType?: string) => {
    ...
    // Upload pending file attachment
    if (hasPendingFile) {
  ...
  // Send voice: stop recording, then send blob
          ...
          <div className="flex-1 flex flex-col rounded-2xl border bg-card shadow-xl overflow-hidden">
            ...
            {authLoading ? (
              ...
              <>
                ...
                {/* Messages */}
                <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4 bg-background/40" style={{ minHeight: 340 }}>
                  {messages.length === 0 && (
                  ...
                  {grouped.map(({ date, msgs }) => (
                    <div key={date}>
                      ...
                      <div className="space-y-2">
                        {msgs.map((msg) => (
                          <MessageBubble key={msg.id} msg={msg} isMe={msg.role === "user"} userName={user.name} />
                        ))}
                      </div>
                    </div>
                  ))}
                  ...
                  <div className="px-4 pt-2 flex items-center gap-2 border-t">
                    <div className="relative inline-block">
                      {pendingFile.type === "image" && (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img src={pendingFile.previewUrl} alt="preview" className="w-16 h-16 object-cover rounded-lg border" />
                      )}
                    ...
                    <p className="text-xs text-muted-foreground truncate max-w-40">{pendingFile.file.name}</p>
                  </div>
                )}
                ...
                {voice.recording && (
                  <div className="px-4 pt-2 border-t flex items-center gap-3">
                    ...
                    <button onClick={voice.cancel} className="text-xs text-muted-foreground underline">Cancel</button>
                  </div>
                )}
                ...
                <div className="p-3 border-t bg-card/80 flex gap-2 items-end">
                  ...
                  <input ref={fileInputRef} type="file" accept="image/*,video/*,audio/*,.pdf,.doc,.docx,.txt,.zip"
                  ...
                  <textarea
                    ...
                    placeholder={voice.recording ? "Recording voice..." : "Type a message..."}
                    rows={1}
                    className="flex-1 text-sm bg-secondary/50 rounded-xl px-4 py-2.5 outline-none resize-none max-h-32 overflow-y-auto"
                    disabled={sending || voice.recording}
                  ...
                  {!input.trim() && !pendingFile ? (
                    // Show mic when nothing typed
                    voice.recording ? (
                      <button onClick={sendVoiceMessage} disabled={sending || uploadingFile}
                        className="bg-red-500 text-white p-2.5 rounded-xl hover:bg-red-600 transition-colors shrink-0"
                        title="Send voice message">
                        <Send className="w-4 h-4" />
                      </button>
                    ) : (
                      <button onClick={voice.start} disabled={sending}
                        className="p-2.5 rounded-xl text-muted-foreground hover:bg-secondary hover:text-foreground transition-colors shrink-0 disabled:opacity-40"
                    ...
                    // Show send when there's text or a file
                    <button onClick={() => sendMessage()}
                      disabled={sending || uploadingFile || voice.recording}
